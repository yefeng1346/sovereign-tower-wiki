...
export function SearchView({ locale, query = "" }: { locale: Locale; query?: string }) {
  ...
  const groups = [...new Set(keywordPages.map((page) => page.category))];
  const normalizedQuery = query.trim().toLowerCase();
  const visiblePages = normalizedQuery ? keywordPages.filter((page) => `${page.keyword} ${page.answer} ${page.category}`.toLowerCase().includes(normalizedQuery)) : keywordPages;
  const visibleGroups = [...new Set(visiblePages.map((page) => page.category))];
  return <>
    <section className="page-intro section-border"><div className="container page-intro-grid"><div><div className="eyebrow">{l.searchEyebrow}<span className="eyebrow-slash">//</span><span>{l.keywordIndex}</span></div><h1>{l.searchTitleA}<br /><span className="accent-text">{l.searchTitleB}</span></h1><p>{l.searchDescription}</p></div><div className="index-counter hud-frame"><strong>{keywordPages.length}</strong><span>{l.indexedQueries}</span><div className="counter-line" /><small>{l.officialCommunity}</small></div></div></section>
